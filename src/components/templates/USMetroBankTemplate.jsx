...
export default function USMetroBankTemplate({
  ...
  const statementEndDate = statementMeta.endDate || '2025-09-30';
  const statementStartDate = statementMeta.startDate || '2025-08-30';
  ...
  const totalCredits = totals?.totalDeposits ?? deposits.reduce((acc, t) => acc + Math.abs(t.amount), 0);
  const totalDebits = totals?.totalWithdrawals ?? debits.reduce((acc, t) => acc + Math.abs(t.amount), 0);

  const endingBal = totals?.endingBalance ?? ((account.startingBalance || 4901.83) + totalCredits - totalDebits);

  // Daily balances
  const dailyBalances = {};
  let running = account.startingBalance || 4901.83;
  ...
  return (
    <div className="us-metro-statement font-sans text-[#111111] bg-white selection:bg-blue-100 print:m-0 print:p-0">
      ...
      >
        ...
        {/* Right Header: Statement Ending 07/31/2026 & Page 1 of 4 */}
        ...
        {/* Customer Address Block (OCR-A / Courier bold monospaced) */}
        ...
        {/* 2D DataMatrix Tracking Barcode (Left of promo) */}
        ...
        >
          {/* Couple photo with peach backdrop */}
          ...
          {/* Right vertical thick black accent bar */}
        ...
        <div
          style={{ position: 'absolute', left: '56px', top: '714px', width: '708px', fontFamily: 'Arial, Helvetica, sans-serif' }}
        ...
        {/* Account Summary Section (Left Half Width matching scan) */}
        <div style={{ position: 'absolute', left: '56px', top: '744px', width: '400px', fontFamily: 'Arial, Helvetica, sans-serif' }}>
          <div className="text-[12px] font-bold text-[#111111] mb-[3px]">
            Account Summary
          </div>
          <table className="w-full text-left text-[11px] border-collapse">
            <thead>
              <tr className="border-b border-[#111111] text-[#111111] font-bold">
                <th className="py-[3px] w-[100px] font-bold">Date</th>
                <th className="py-[3px] font-bold">Description</th>
                <th className="py-[3px] text-right w-[110px] font-bold">Amount</th>
              </tr>
            </thead>
            <tbody>
              <tr className="font-medium text-[#111111]">
                <td className="py-[2.5px] font-bold">{formattedStartDate}</td>
                <td className="py-[2.5px]">Beginning Balance</td>
                <td className="py-[2.5px] text-right font-bold">${(account.startingBalance || 389218.13).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
              </tr>
              <tr className="text-[#111111]">
                <td className="py-[2.5px]"></td>
                <td className="py-[2.5px]">{deposits.length} Credit(s) This Period</td>
                <td className="py-[2.5px] text-right font-bold">${totalCredits.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
              </tr>
              <tr className="text-[#111111]">
                <td className="py-[2.5px]"></td>
                <td className="py-[2.5px]">{debits.length} Debit(s) This Period</td>
                <td className="py-[2.5px] text-right font-bold">${totalDebits.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
              </tr>
              <tr className="font-bold text-[#111111]">
                <td className="py-[2.5px]">{formattedEndDate}</td>
                <td className="py-[2.5px]">Ending Balance</td>
                <td className="py-[2.5px] text-right">${endingBal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
              </tr>
            </tbody>
          </table>
        </div>

        {/* Transaction Detail Table (Other Credits or Other Debits based on transactions) */}
        <div style={{ position: 'absolute', left: '56px', top: '876px', width: '708px', fontFamily: 'Arial, Helvetica, sans-serif' }}>
          <div className="text-[12px] font-bold text-[#111111] mb-[3px]">
            {deposits.length > 0 && debits.length === 0 ? 'Other Credits' : 'Other Debits'}
          </div>
          <table className="w-full text-left text-[11px] border-collapse">
            <thead>
              <tr className="border-b border-[#111111] text-[#111111] font-bold">
                <th className="py-[3px] w-[110px] font-bold">Date</th>
                <th className="py-[3px] font-bold">Description</th>
                <th className="py-[3px] text-right w-[140px] font-bold">Amount</th>
              </tr>
            </thead>
            <tbody>
              {deposits.length > 0 && debits.length === 0 ? (
                deposits.map((tx, idx) => (
                  <tr key={idx} className="font-medium text-[#111111]">
                    <td className="py-[2.5px] font-bold">{formatMDY(tx.date)}</td>
                    <td className="py-[2.5px] uppercase">{tx.description}</td>
                    <td className="py-[2.5px] text-right font-bold">${Math.abs(tx.amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  </tr>
                ))
              ) : debits.length > 0 ? (
                debits.map((tx, idx) => (
                ...
                <tr className="font-medium text-[#111111]">
                  <td className="py-[2.5px] font-bold">{formattedEndDate}</td>
                  <td className="py-[2.5px] uppercase">INTEREST CREDIT</td>
                  <td className="py-[2.5px] text-right font-bold">$578.47</td>
                </tr>
              )}
            </tbody>
          </table>
          <div className="text-right text-[11px] text-[#111111] font-medium border-t border-[#111111] pt-[2px]">
            {deposits.length > 0 && debits.length === 0
              ? `${deposits.length} item(s) totaling $${totalCredits.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
              : `${debits.length} item(s) totaling $${totalDebits.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
          </div>
        </div>

        {/* Page 1 Footer: FDIC & Equal Housing Lender Logos */}
        <div style={{ position: 'absolute', left: '72px', top: '962px' }}>
          <FdicLogo className="h-[22px]" color="#274D7E" showSubtext={true} />
        </div>
        <div style={{ position: 'absolute', left: '592px', top: '952px' }}>
          <EqualHousingLenderLogo className="h-[34px]" color="#274D7E" />
        </div>
      </div>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
            <span>CSTMTADV 1071 0001 124 07 20260801 PG 1 OF 2</span>
            <span>0-0</span>
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-12">
            {/* Left: Checks Outstanding columns (3 pairs of Date/# and Amount) */}
            <div className="col-span-8 border-r-[1.8px] border-[#1e3a68]">
              ...
              {/* 11 blank check rows with dashed interior lines and solid column dividers */}
              {[...Array(11)].map((_, i) => (
        ...
        {/* Electronic Transfers Error Notice & Funds Availability (Single Master Framed Box) */}
        <div
        ...
        >
          ...
          {/* Bottom Sub-Section: FUNDS AVAILABILITY */}
      ...
      >
        ...
        <div style={{ position: 'absolute', left: '56px', top: '108px', width: '335px' }}>
          <div className="text-[12px] font-bold text-[#111111] mb-[3px]">
            Daily Balances
          </div>
          <table className="w-full text-left text-[11px] border-collapse">
            <thead>
              <tr className="border-b border-[#111111] text-[#111111] font-bold">
                <th className="py-[3px] w-[140px] font-bold">Date</th>
                <th className="py-[3px] text-right font-bold">Amount</th>
              </tr>
            </thead>
            <tbody>
              {dailyEntries.slice(0, 15).map(([d, b], idx) => (
                <tr key={idx} className="font-medium text-[#111111]">
                  <td className="py-[2.5px] font-bold">{formatMDY(d)}</td>
                  <td className="py-[2.5px] text-right font-bold">${Number(b).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={{ position: 'absolute', left: '56px', top: '225px', width: '708px' }}>
          <div className="text-[12px] font-bold text-[#111111] mb-[4px]">
            Overdraft and Returned Item Fees
          </div>
          <table className="w-full border-[1.8px] border-[#1e3a68] text-left text-[11px] border-collapse">
            <thead>
              <tr className="border-b-[1.8px] border-[#1e3a68] bg-white font-bold text-[#111111]">
                <th className="py-[4px] px-[8px] border-r-[1.8px] border-[#1e3a68]"></th>
                <th className="py-[4px] px-[8px] text-center border-r-[1.8px] border-[#1e3a68] w-[200px] font-bold">Total for this period</th>
                <th className="py-[4px] px-[8px] text-center w-[200px] font-bold">Total year-to-date</th>
              </tr>
            </thead>
            <tbody className="divide-y-[1.8px] divide-[#1e3a68]">
              <tr>
                <td className="py-[4px] px-[8px] border-r-[1.8px] border-[#1e3a68] font-bold text-[#111111]">Total Overdraft Fees</td>
                <td className="py-[4px] px-[8px] text-right border-r-[1.8px] border-[#1e3a68] font-bold text-[#111111] pr-4">$0.00</td>
                <td className="py-[4px] px-[8px] text-right font-bold text-[#111111] pr-4">$0.00</td>
              </tr>
              <tr>
                <td className="py-[4px] px-[8px] border-r-[1.8px] border-[#1e3a68] font-bold text-[#111111]">Total Returned Item Fees</td>
                <td className="py-[4px] px-[8px] text-right border-r-[1.8px] border-[#1e3a68] font-bold text-[#111111] pr-4">$0.00</td>
                <td className="py-[4px] px-[8px] text-right font-bold text-[#111111] pr-4">$0.00</td>
              </tr>
            </tbody>
        ...
        {/* 2D DataMatrix Tracking Barcode on Page 3 (Matching Left Margin) */}
        <div style={{ position: 'absolute', left: '26px', top: '375px' }}>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
            <span>CSTMTADV 1071 0001 124 07 20260801 PG 2 OF 2</span>
            <span>0-0</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', marginTop: '1px' }}>
            <span>01026215</span>
